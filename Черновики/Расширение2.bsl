...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ЗаполнениеМКПриНаличииВРегистре(Клиент);
	//Лысенко
	
КонецПроцедуры

//Лысенко
#Область УдалитьВСледующийРаз
//&НаСервере
//Процедура ЗаполнениеМКПриНаличииВРегистре(Клиент)
//	
//	Запрос = Новый Запрос;
...
//	|	ДокументыФизическихЛиц.КемВыданМатКап КАК КемВыданМатКап,
//	|	ДокументыФизическихЛиц.НомерРешенияВыдачи КАК НомерРешенияВыдачи,
//	|	ДокументыФизическихЛиц.Сумма КАК Сумма
//	|ИЗ
//	|	РегистрСведений.ДокументыФизическихЛиц КАК ДокументыФизическихЛиц
//	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Партнеры КАК Партнеры
//	|		ПО ДокументыФизическихЛиц.Физлицо = Партнеры.ОсновноеКонтактноеЛицо
//	|ГДЕ
//	|	ДокументыФизическихЛиц.ВидДокумента = &ВидДокумента
//	|	И Партнеры.Ссылка = &Ссылка";
//	Запрос.УстановитьПараметр("Ссылка", Клиент);
//	Запрос.УстановитьПараметр("ВидДокумента", ПредопределенноеЗначение("Справочник.ВидыДокументовФизическихЛиц.СертификатМатеринскогоКапитала"));
//	Результат = Запрос.Выполнить();
//	Если НЕ Результат.Пустой() Тогда
//		ТЗ = Результат.Выгрузить();
//		КолонкаСерия = ТЗ.ВыгрузитьКолонку("Серия");
//		КолонкаНомер = ТЗ.ВыгрузитьКолонку("Номер");
//		КолонкаДатаВыдачи = ТЗ.ВыгрузитьКолонку("ДатаВыдачи");
//		КолонкаКемВыдан = ТЗ.ВыгрузитьКолонку("КемВыданМатКап");
//		КолонкаНомерРешенияВыдачи = ТЗ.ВыгрузитьКолонку("НомерРешенияВыдачи");
//		КолонкаСумма = ТЗ.ВыгрузитьКолонку("Сумма");
//		Серия = КолонкаСерия[0];
//		Номер = КолонкаНомер[0];
//		ДатаВыдачи = КолонкаДатаВыдачи[0];
//		КемВыдан = КолонкаКемВыдан[0];
//		НомерРешенияВыдачи = КолонкаНомерРешенияВыдачи[0];
//		Сумма = КолонкаСумма[0];
//		Объект.МКСерия = Серия;
//		Объект.МКНомер = Номер;
//		Объект.МКДатаВыдачи = ДатаВыдачи; 
//		Объект.МКДатаРешения = ДатаВыдачи;
//		Объект.МККемВыдан = КемВыдан;
//		Объект.МКНомерРешения = НомерРешенияВыдачи;
//		Объект.МКСумма = Сумма;
//	КонецЕсли;
//	
//КонецПроцедуры
#КонецОбласти
...
Процедура УдалитьПокупателяСервер(Знач ЭлементИмя)
	...
	УдаляемыеРеквизиты.Добавить("Доля" + ТекНомер);  
	//++ Абидинов по задаче FLT-50
	УдаляемыеРеквизиты.Добавить("СовместноДолевая" + ТекНомер);
	//- Абидинов по задаче FLT-50
	ИзменитьРеквизиты(, УдаляемыеРеквизиты);
...
//Лысенко
&НаКлиенте
Процедура РаспознатьИЗаполнитьМК(Команда)
	
	Клиент = Объект.Клиент;
	РаспознатьСертификат(Клиент);			
	
КонецПроцедуры

&НаСервере
Процедура РаспознатьСертификат(Клиент)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Партнеры.ОсновноеКонтактноеЛицо КАК ФизЛицо
	|ПОМЕСТИТЬ Выборка_ФизЛицо
	|ИЗ
	...
	|	Выборка_ФизЛицо.ФизЛицо КАК ФизЛицо,
	|	ДокументыФизическихЛиц.Серия КАК Серия,
	|	ДокументыФизическихЛиц.Номер КАК Номер,
	|	ДокументыФизическихЛиц.ДатаВыдачи КАК ДатаВыдачи,
	|	ДокументыФизическихЛиц.НомерРешенияВыдачи КАК НомерРешенияВыдачи,
	|	ДокументыФизическихЛиц.КемВыданМатКап КАК КемВыдан,
	|	ДокументыФизическихЛиц.Сумма КАК Сумма
	|ИЗ
	|	Выборка_ФизЛицо КАК Выборка_ФизЛицо
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДокументыФизическихЛиц КАК ДокументыФизическихЛиц
	|		ПО Выборка_ФизЛицо.ФизЛицо = ДокументыФизическихЛиц.Физлицо
	|ГДЕ
	|	ДокументыФизическихЛиц.ВидДокумента = &ВидДокумента";
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		|	РеализацияНедвижимостиПрисоединенныеФайлы.Ссылка КАК Ссылка,
		|	РеализацияНедвижимостиПрисоединенныеФайлы.Расширение КАК Расширение
		|ИЗ
		|	Справочник.РеализацияНедвижимостиПрисоединенныеФайлы КАК РеализацияНедвижимостиПрисоединенныеФайлы
		|ГДЕ
		|	РеализацияНедвижимостиПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайла
		|	И (РеализацияНедвижимостиПрисоединенныеФайлы.Наименование ПОДОБНО &Первый
		|	ИЛИ РеализацияНедвижимостиПрисоединенныеФайлы.Наименование ПОДОБНО &Второй
		|	ИЛИ РеализацияНедвижимостиПрисоединенныеФайлы.Наименование ПОДОБНО &Третий)";
		Запрос.УстановитьПараметр("ВладелецФайла", Объект.Ссылка);
		Запрос.УстановитьПараметр("Первый", "%" + "серт" + "%");
		Запрос.УстановитьПараметр("Второй", "%" + "материн" + "%");
		Запрос.УстановитьПараметр("Третий", "%" + "капитал" + "%");
		Результат = Запрос.Выполнить();
		...
		Если НЕ Результат.Пустой() Тогда
			...
			КонецЦикла;
			Структура = ПрисоединенныеФайлы.ПолучитьДанныеФайла(Файл, Новый УникальныйИдентификатор, Истина);
			СсылкаНаФайл = Структура.СсылкаНаДвоичныеДанныеФайла;
			
			Структура = УПДК_ФизическиеЛица.РаспознатьСертификатНаМатКап(СсылкаНаФайл, Расширение);
			
			Объект.МКСерия = Структура.Серия;
			Объект.МКНомер = Структура.Номер;
			Объект.МКДатаВыдачи = Структура.ДатаВыдачи; 
			Объект.МКДатаРешения = Структура.ДатаВыдачи;
			Объект.МККемВыдан = Структура.КемВыдан;
			Объект.МКНомерРешения = Структура.НомерРешенияОВыдаче;
			Объект.МКСумма = Структура.Сумма;
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Загрузка выполнена")
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Необходимо загрузить документ сертификата материнского капитала во вкладку ""Присоединенные файлы""");
		КонецЕсли
	Иначе
		Структура = Новый Структура;
		Пока Выборка.Следующий() Цикл
			Объект.МКСерия = Выборка.Серия;
			Объект.МКНомер = Выборка.Номер;
			Объект.МКДатаВыдачи = Выборка.ДатаВыдачи; 
			Объект.МКДатаРешения = Выборка.ДатаВыдачи;
			Объект.МККемВыдан = Выборка.КемВыдан;
			Объект.МКНомерРешения = Выборка.НомерРешенияВыдачи;
			Объект.МКСумма = Выборка.Сумма;
		КонецЦикла;	
	КонецЕсли;
...
Процедура УдалитьИсториюВыбора()  
	Выборка = Документы.РеализацияНедвижимости.Выбрать();
	Пока Выборка.Следующий() цикл
		ХранилищеСистемныхНастроек.Удалить("ИсторияВыбора", , Выборка.ВидПравовогоДокумента);  
	КонецЦикла; 
КонецПроцедуры 
