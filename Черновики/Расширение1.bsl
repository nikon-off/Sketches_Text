...
//  ЭлементДатаИсполнения  - ПолеВвода - поле с датой исполнения задачи.
//
Процедура ФормаЗадачиПриСозданииНаСервере(ФормаЗадачи, ЗадачаОбъект,
	...
	Если ЗадачаОбъект.Выполнена Тогда
		...
		//УПДК+
		НСтр("ru ='Задача %1 %2 пользователем %3.'"),
		?(ЗадачаОбъект.CRM_Переадресована, "перенаправлена", "выполнена"),
		//УПДК 
...
Процедура УстановитьПометкуУдаленияЗадач(БизнесПроцессСсылка, ПометкаУдаления) Экспорт
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			// +CRM
			Если ЗначениеЗаполнено(Выборка.Ссылка.БизнесПроцесс)
				И ТипЗнч(Выборка.Ссылка.БизнесПроцесс) = Тип("БизнесПроцессСсылка.CRM_БизнесПроцесс") И ПометкаУдаления Тогда
...
Процедура ПроконтролироватьЗадачи() Экспорт
	...
	//УПДК+
	УстановитьПривилегированныйРежим(Истина);
	Получатели = НаборПисемПоАдресатам.ВыгрузитьКолонку("Получатель");
	Запрос = Новый Запрос("ВЫБРАТЬ
	...
	|ГДЕ
	|	CRM_ОповещенияПользовательскиеНастройки.Пользователь В(&Пользователь)
	|	И CRM_ОповещенияПользовательскиеНастройки.ВидОповещения = ЗНАЧЕНИЕ(Справочник.CRM_ВидыОповещений.ПросроченныеНеЗавершенныеСобытияЗадачи)
	|	И CRM_ОповещенияПользовательскиеНастройки.ЭлектроннаяПочта = ИСТИНА");
	Запрос.УстановитьПараметр("Пользователь", Получатели);
	РазрешенныеАдресаты = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Пользователь");
	//УПДК
	
	Для Каждого ПисьмоИзНабора Из НаборПисемПоАдресатам Цикл
		
		//УПДК+
		Если РазрешенныеАдресаты.Найти(ПисьмоИзНабора.Получатель) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		//УПДК
...
//   БизнесПроцесс - ОбъектМетаданных - метаданные бизнес-процесса, наборы значений доступа
...
Процедура ПриПолученииСпискаШаблонов(Шаблоны) Экспорт
	...
	Шаблоны.Добавить("СтартОтложенныхПроцессов");
...
Функция ВыбратьНовыеЗадачиПоИсполнителям(Знач ДатаВремяОт, Знач ДатаВремяПо)
	...
	|	Исполнитель");
	//УПДК+
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВЫБРАТЬ РАЗРЕШЕННЫЕ", "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ");
	//УПДК
...
Процедура РаспределитьНаИсполнителя(РольИсполнителя, КартаМаршрута, Предмет, Исполнитель) Экспорт
	...
	Если ТипЗнч(Предмет) = Тип("ДокументСсылка.РеализацияНедвижимости") Тогда 
		...
		ИсполнительЗакрепленногоКлиента = ПолучитьПоследнегоБрокераПоТекущемуКлиенту(Клиент);
	...
	Если ТипЗнч(Предмет) = Тип("ДокументСсылка.РеализацияНедвижимости") Тогда
		//СтруктураЗаполнения = Новый Структура("Брокер", Исполнитель);
		//УПДК_ОбщегоНазначения.ЗаписатьДанныеВОбъектСБлокировкой(Предмет, СтруктураЗаполнения);
...
Процедура УстановитьСрокОтветсвенности(Договор, Пользователь) Экспорт
...
Функция ПолучитьПоследнегоБрокераПоТекущемуКлиенту(Клиент, Ссылка = Неопределено, РольИсполнителя = Неопределено, КартаМаршрута = Неопределено) Экспорт
...
Функция СписокЗначенийДопОтбораПорядкаРаспределения() Экспорт
	...
	СписокЗначений.Вставить(1, "Несовершеннолетние", "Несовершеннолетние покупатели", Ложь);
...
#Область ЗапускФономБпКц

Процедура ЗапуститьБизнесПроцессыКЦВФоне(Сделка) Экспорт
	
	СтруктураПараметров = Новый Структура("Сделка", Сделка); 
	ЗапуститьФоновоеВыполнение("БизнесПроцессыИЗадачиСервер.ЗапуститьБизнесПроцессыКЦ(Сделка);", СтруктураПараметров);
...
Функция ЗапуститьФоновоеВыполнение(ТекстПроцедуры,СтруктураПараметров = Неопределено) Экспорт
	...
	Задание = ФоновыеЗадания.Выполнить("БизнесПроцессыИЗадачиСервер.ФоноваяПроцедура",ПараметрыВыполнения,УникальныйИдентификатор,"ЗапуститьБизнесПроцессыКЦВФоне"); 
...
Процедура ЗапуститьБизнесПроцессыКЦ(Сделка) Экспорт
	
	Попытка
		Запрос = Новый Запрос;
		Запрос.Текст = "
		|ВЫБРАТЬ
		|	ТематикаПоНаправлениямБизнеса.КартаМаршрута КАК КартаМаршрута
		|ИЗ
		|	Документ.Сделка.ПотребностиКЦ КАК ПотребностиКЦ
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.Дев_ТематикаПоНаправлениямБизнеса КАК ТематикаПоНаправлениямБизнеса
		|		ПО (ТематикаПоНаправлениямБизнеса.ВидБизнеса = ПотребностиКЦ.ВидБизнесПроцесса)
		|			И (ТематикаПоНаправлениямБизнеса.Тема = ПотребностиКЦ.ВидКонсультации)
		|			И (ТематикаПоНаправлениямБизнеса.ПодТема = ПотребностиКЦ.ПодВидКонсультации)
		|ГДЕ
		|	ПотребностиКЦ.Ссылка = &Ссылка";
		...
		Если РезультатЗапроса.Пустой() Тогда
			ТекстОтвета = СтрШаблон("По %1 отсутствуют карты маршрута", Сделка);
		...
		Пока Выборка.Следующий() Цикл
			СтруктураОснования = Новый Структура("Основание, КартаМаршрута", Сделка, Выборка.КартаМаршрута);
